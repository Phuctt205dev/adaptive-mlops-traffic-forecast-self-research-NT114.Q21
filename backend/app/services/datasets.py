...
from backend.app.services import dvc_versioning
...
def upload_region_dataset(
    ...
    if dataset.status == DatasetStatus.VALID:
        try:
            dvc_report = dvc_versioning.version_dataset(
                region_id=region_id,
                dataset_id=dataset.id,
                filename=Path(filename).name,
                content=content,
            )
            dataset.dvc_rev = dvc_report.get("rev")
            report["dvc"] = dvc_report
            storage.put_object(
                settings.dataset_bucket,
                report_key,
                json.dumps(report, indent=2, default=str).encode("utf-8"),
            ...
            db.commit()
            db.refresh(dataset)
        except dvc_versioning.DVCVersioningError as error:
            db.rollback()
            logger.warning(
                "DVC versioning failed for dataset %s: %s",
                dataset.id,
                error,
            )
            report["dvc"] = {
                "enabled": settings.dvc_enabled,
                "rev": None,
                "error": str(error),
            }
            if settings.dvc_required:
                raise ApplicationError(
                    "dataset_dvc_versioning_failed",
                    "Dataset was valid, but DVC versioning failed.",
                    500,
                ) from error
